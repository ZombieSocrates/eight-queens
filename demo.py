...
def make_chess_board(dim = 8, queen_loc_seed = None):
    ...
    REPLACED BY __init__ of chessBoard class
    '''
    chess_board = []
...
def display_chess_board(chess_board):
    ...
    Moved as a method of chessBoard class
    '''
...
def solution_shortdoc(board_dim, solved_bool, n_steps):
    if solved_bool:
        print(f"We solved the {board_dim}-queens problem in {n_steps} steps!")
    else:
        print(f"We took {n_steps} steps and found no solution...")
...
def solve_many_boards(seed_list, dim_each = 8, steps_each = 50,
    ...
    for i, s in enumerate(seed_list):
        ...
        solved, n_steps = solve_queens_problem(chess_board = s_board, 
            max_steps = steps_each, verbose = verbose, 
            stop_each = stop_each)
        solution_shortdoc(dim_each, solved, n_steps)
        print("-" * bcount, "\n")
...
if __name__ == "__main__":
    ...
    # board_dim = 25
    # dat_board = make_chess_board(dim = board_dim, queen_loc_seed = 42)
    # soln_bool, soln_score = solve_queens_problem(chess_board = dat_board, 
    #     max_steps = 100, verbose = True, stop_each = 10)
    # solution_shortdoc(board_dim, soln_bool, soln_score)

    # TODO: 
    # 1. Create a separate chess_board class and then a base_solver class.
